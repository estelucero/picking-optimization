    ...
    if (savedMappings) {
      ...
      if (currentMapping) {
        try {
          ...
          const matching = parsed.find((m: ProductMapping) => 
            m.coordinates.length === coords.length &&
            m.coordinates.every((c: Coordinate) => 
              coords.some((c2: Coordinate) => c.id === c2.id && c.x === c2.x && c.y === c2.y)
            )
          )
          if (matching) {
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-2">Experimentation</h1>
          <p className="text-slate-600 dark:text-slate-400">
            Run simulations based on product mappings and operational parameters
          </p>
        </div>
        {mappings.length === 0 && (
          <Link href="/product-mapping">
            <Button className="bg-blue-600 hover:bg-blue-700 text-white gap-2">
              Create Product Mapping <ArrowRight className="w-4 h-4" />
            </Button>
          </Link>
        )}
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-slate-600 dark:text-slate-400">Loading mappings...</div>
        </div>
      ) : mappings.length === 0 ? (
        <Card className="p-8 text-center bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">No Product Mappings Found</h2>
          <p className="text-slate-600 dark:text-slate-400 mb-6">
            You need to create a product mapping first before running experiments.
          </p>
          <Link href="/product-mapping">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-3">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Available Product Mappings</h2>
              {mappings.map((mapping) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white">{mapping.name}</h3>
          ...
          <div>
            <ExperimentRunPanel selectedMapping={selectedMapping || undefined} />
          </div>
        </div>
      )}
    </div>
