  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Simulation Results</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Analysis of operational efficiency across different worker configurations
          </p>
        </div>
        <Button
          onClick={handleBack}
      ...
      {!isLoading && results.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-2xl p-6 shadow-lg">
            <p className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Data Points</p>
            <p className="text-4xl font-bold text-slate-900 dark:text-white mt-3">{results.length}</p>
          </div>
          <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-2xl p-6 shadow-lg">
            <p className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Workers Range</p>
            <p className="text-4xl font-bold text-slate-900 dark:text-white mt-3">
              {results[0]?.operarios || 0} - {results[results.length - 1]?.operarios || 0}
            </p>
          </div>
          <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-2xl p-6 shadow-lg">
            <p className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Max Time</p>
            <p className="text-4xl font-bold text-slate-900 dark:text-white mt-3">
      ...
      {isLoading && (
        <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-2xl p-12 flex flex-col items-center justify-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-4 font-medium">Loading results...</p>
        </div>
      )}

      {!isLoading && results.length === 0 && (
        <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-2xl p-12 text-center">
          <p className="text-slate-600 dark:text-slate-400 mb-6 text-lg">No data to display</p>
          <Button onClick={handleBack} className="bg-blue-600 hover:bg-blue-700 text-white">Back to Experimentation</Button>
        </div>
      )}
    </div>
  )
