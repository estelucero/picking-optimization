...
def producto_liviano():
    return Producto(codigo="SKU-001", nombre="Producto Liviano", peso=2.0, x=0, y=0)


@pytest.fixture
def producto_pesado():
    return Producto(codigo="SKU-002", nombre="Producto Pesado", peso=5.0, x=0, y=0)
...
class TestCarroAgregarProducto:
    ...
    def test_no_puede_agregar_producto_que_supera_capacidad(self, producto_pesado):
        carro = Carro(capacidad_max_peso=3.0)
...
class TestCarroPeso:
    ...
    def test_capacidad_restante_cero_cuando_lleno(self, producto_liviano):
        carro = Carro(capacidad_max_peso=10.0)
        carro.agregar_producto(producto_liviano, 5)
